...
public class TC05_ValidateBookingPage {
	...
	@Test(priority=29)
	private void check_Overview() {
		try {
			if (manager.getConfirmation().getIteneraryBtn().isDisplayed()) {
				if (manager.getConfirmation().getIteneraryBtn().isEnabled()) {
					manager.getConfirmation().getIteneraryBtn().click();
					System.out.println("user clicked the button successfully");
				}
			}

			if (manager.getBookedPage().getCheckBox().isDisplayed()) {
				if (manager.getBookedPage().getCheckBox().isEnabled()) {

					manager.getBookedPage().getCheckBox().click();
					System.out.println("user clicked radio btn successfully");

				}
			}

			Thread.sleep(2000);
			if (manager.getBookedPage().getClickonPage().isDisplayed()) {
				if (manager.getBookedPage().getClickonPage().isEnabled()) {
					manager.getBookedPage().getClickonPage().click();

					System.out.println("user clicked cancel button successfully");
				}
			}

			base.acceptAlert();

			if (manager.getBookedPage().getLogout().isDisplayed()) {
				if (manager.getBookedPage().getLogout().isEnabled()) {
					manager.getBookedPage().getLogout().click();
					System.out.println("user clicked the  log out button successfully");
